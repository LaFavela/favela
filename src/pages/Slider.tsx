...
const Slider = () => {
  ...
  const items = [Konten(), Konten(), Konten(), Konten()];
  ...
  return (
    <div>
      <div className='flex flex-row ml-20 pb-5 gap-8' style={{ scrollBehavior: 'smooth', overflowX: 'hidden' }}>
        <div className=" mt-5 ml-3 slider-container" style={{ display: 'flex', flexDirection: 'row', transition: 'transform 0.5s', transform: `translateX(-${activeIndex * 31}rem)` }}>
          {items.map((item, index) => (
            <div
              key={index}
              className={`w-[31rem] h-[16rem] rounded-[1.5625rem] bg-white drop-shadow-landingShado ${index === activeIndex ? 'opacity-100' : 'opacity-100'}`}
      ...
      <div className='flex justify-center items-center gap-5'>
        <button className='bg-gray-500 rounded-xl w-20 h-10' onClick={handlePrev}>
          Prev
        </button>
        <button className='bg-gray-500 rounded-xl w-20 h-10' onClick={handleNext}>
          Next
        </button>
      </div>
    </div>
  ...
  return (
    <div>
      <div className='ml-7 mt-7 w-[25.5rem]'>
        ...
        <p className='font-light text-[0.9rem] mt-4'>
