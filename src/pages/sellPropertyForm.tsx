...
export default function SellPropertyForm() {
   const [saranaLain, setSaranaLain] = useState<string[]>([""] as string[]);
   const [name, setName] = useState<string>();
   const [marker, setMarker] = useState({ lat: 0, lng: 0 } as Marker);
   const [description, setDescription] = useState<string>();
   const [developer, setDeveloper] = useState<string>();
   const [certificate, setCertificate] = useState<string>();
   const [finished, setFinished] = useState<number>();
   const [listrik, setListrik] = useState<number>();
   const [kamarTidur, setKamarTidur] = useState<number>();
   const [kamarMandi, setKamarMandi] = useState<number>();
   const [luas, setLuas] = useState<number>();
   const [hargaJual, setHargaJual] = useState();
   const [hargaSewaBulan, setHargaSewaBulan] = useState<number>();
   const [provinsi, setProvinsi] = useState<string>();
   const [kota, setKota] = useState<string>();
   const [kecamatan, setKecamatan] = useState<string>();
   const [kodePos, setKodePos] = useState<number>();
   ...
   const handleDesignDelete = (index: number) => {
      setDetailImg((prevImages: any[]) => {
         return prevImages.filter(
            (imageInfo: any) => imageInfo.index !== index
         );
      });
   };
         ...
         <div className="flex justify-center ">
            <form onSubmit={handleSubmit}>
               <div className="mb-11 mt-[2.625rem] space-y-[2.4375rem]">
                        ...
                        <div className="flex justify-between ">
                           ...
                           <div className="flex flex-col">
                              {saranaLain.map((sarana, index) => (
                                 <input
                                    key={index}
                                    className="mt-[0.625rem] h-[2.5rem] w-[53.875rem] rounded-[7px] border-[1px] border-gold bg-white pl-[1.25rem] text-gold focus:border-[#B17C3F] focus:outline-none focus:ring-1 focus:ring-[#B17C3F] "
                                    placeholder="Sarana Lain"
                                    value={sarana}
                                    onChange={(e) =>
                                       handleChangeSaranaLain(
                                          index,
                                          e.target.value
                                       )
                                    }
                        ...
                        <div className="flex justify-between ">
                           ...
                           <div>
                              <input
                                 ...
                                 onChange={(e: any) =>
                                    setHargaSewaBulan(e.target.value)
                                 }
                              />
                           </div>
                        </div>
                  ...
                  {/* Lokasi */}
                  <div className="flex h-max w-[78.93rem] justify-center rounded-[25px] bg-white pb-10 drop-shadow-[0_0px_3px_rgba(0,0,0,0.25)]">
                     <div className="mt-[2rem]  flex w-[67.625rem] flex-col space-y-[2.43rem]">
                        {/* Lokasi Tittle */}
                        <p className="text-[1.31rem] font-semibold text-[#b17c3f] ">
                           Lokasi
