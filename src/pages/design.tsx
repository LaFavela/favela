...
>) {
	...
	const handleTagDelete2 = (value: number) => {
		const updatedTags = budget.filter((tag) => tag !== value);
		setbudget(updatedTags); 
	};
						...
						<div className="flex h-[55px] w-full bg-white justify-between rounded-full border-2">
							<div className=" flex">
								...
								<div className="tags-input mt-3 pl-3 flex gap-x-2">
									...
									<div className="flex flex-row gap-2">
										<AnimatePresence>
											{budget.map((tag, index) => (
												<motion.li
													initial={{ scale: 0 }}
													animate={{ scale: 1 }}
													exit={{ scale: 0 }}
													className="flex h-[30px] max-w-fit flex-row justify-between items-center rounded-full bg-[#E4D1BC]"
													key={index}
												>
								...
								<div className="flex gap-x-6">
										...
										<div className="flex flex-col gap-x-2 space-y-1 ">
											{style.map((style, index) => (
												<div key={index} className="">
													<button
														value={style.value}
														onClick={() => handleStyleTagClick(style.value)}
									...
									<div className="border-l-2 pr-3 pl-4 relative">
										...
										<div className="flex flex-col justify-center items-center mt-[5px] ">
											<div className="flex">
												<p className="absolute ml-2 mt-1 font-normal text-gold text-[0.7rem]">
													Rp.
												</p>
												<form
