...
export default function DesignProduct({
	...
	const [initial_type, setInitial_type] = useState<number>(
		design?.property_type!,
	);
	const [initial_style, setInitial_style] = useState<number>(
		design?.property_style!,
	);
	...
	useEffect(() => {
		const fetch = async () => {
			const data = await supabase.rpc("get_user_role_name");
			...
			if (property_style) {
				setProperty_style(property_style);
			}
	...
	const [img, setImg] = useState(
		design.preview_image !== null ? design.preview_image[0] : " ",
	);
					...
					<div className="flex h-[34.4375rem] w-[33.625rem] ">
						<div className="flex h-full w-full flex-col space-y-5">
							<div className="flex aspect-square ">
								<div
									className="relative h-full w-full flex-auto  bg-black rounded-3xl"
									onClick={() => {
										setSrcIMG(img);
										setShowIMG(true);
									}}
							...
							<div className="">
								...
								>
									{design.preview_image != null &&
										design.preview_image.map((image, index) => (
											<div
												key={index}
											...
											>
												...
												<div className="bg-black rounded-3xl overflow-hidden mx-2 drop-shadow-sm">
													<Image
														className={
															currImg === index
																? " aspect-square w-[100%] hover:opacity-70 rounded-3xl border-4 border-[#b17c3f] object-cover"
																: " aspect-square w-[100%] hover:opacity-70 rounded-3xl  "
														}
														src={image}
														alt={""}
														// fill={true}
						...
						<div className="">
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center space-x-[0.9375rem]">
									<p className="text-[1.25rem] font-medium">{design.name}</p>
									{property_type != null && (
							...
							<div className="mt-[0.4rem] flex items-center  space-x-4">
								...
								<div className="flex items-center space-x-2 ">
									...
									<p className="mt-1 text-[0.75rem]">
										{design.bathroom_count} Bathroom
									</p>
								</div>
								<div className="flex items-center space-x-2 ">
									<svg
							...
							<div className="space-y-2">
								...
								<div className="space-y-2">
									...
									<div className="flex space-x-3">
										{design.floor_plan_image != null &&
											design.floor_plan_image.map((data, idx) => (
												<div
													key={idx}
												...
												<div className="space-y-2">
													...
													<div>
														<Rating
															...
															emptyIcon={
																<CircleIcon
																	style={{ opacity: 0.55, fontSize: 13 }}
																/>
															}
														/>
													</div>
									...
									{review.length > 3 && (
										<div className="flex justify-center cursor-pointer ">
											{visibleItems < review.length ? (
												...
												<p
													className="text-[#B17C3F] text-[0.8rem] hover:bg-gray-100 rounded-lg p-1 px-2 mt-4"
													onClick={() => {
														setVisibleItems(3);
													}}
												>
