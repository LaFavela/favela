...
import { Users, RoleSelect } from "@prisma/client";
...
export const getServerSideProps: GetServerSideProps<{ data: Users[] }> = async (
   ...
   const data: Users[] = await useGet("/admin/listUser", {
      authorization: token as string,
   });
...
export default function Dashboard({ data, token }: ProfileProps) {
   const handleDelete = async (id: string) => {
      const user = await usePost("/admin/deleteUser", {
         authorization: token as string,
         id: id,
      });
                  ...
                  {data.map((data, idx) => (
                     <div key={idx} className="mt-3 flex w-full justify-center">
                        ...
                        <div className="flex w-[13.4375rem] items-center justify-center space-x-4 py-2">
                           <Link href={`/admin/settings/${data.id}`}>
                           ...
                           <button
                              onClick={async () => handleDelete(data.id)}
                              className="flex w-[4.25rem] items-center justify-center space-x-1 rounded-lg bg-[#DC7567] p-[0.35rem] hover:bg-[#e09388]"
                           >
                              <svg
                                 width="19"
                                 height="18"
                              ...
                              <p className="text-[0.625rem] text-white">
                                 Delete
                              </p>
                           </button>
                        </div>
